...
class MagicTiles(Screen):
    menu = Menu()
    perspective_point_x = NumericProperty(9)
    perspective_point_y = NumericProperty(9)
    ...
    def __init__(self,is_desktop, **kwargs):
        super(MagicTiles, self).__init__(**kwargs)
        self.name = 'magic_tile'
    ...
    def on_touch_down(self, touch):
        if not self.is_game_over and self.is_game_started:
            if touch.x < self.width / 2:
                ...
                self.c_speed_x = -self.speed_x
        return super(MagicTiles, self).on_touch_down(touch)
